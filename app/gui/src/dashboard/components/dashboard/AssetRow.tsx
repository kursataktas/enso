...
export const AssetRow = React.memo(function AssetRow(props: AssetRowProps) {
  const { isKeyboardSelected, isOpened, select, state, columns, onClick, path } = props
  const { id, parentId, depth, hidden: hiddenRaw, grabKeyboardFocus, initialAssetEvents } = props
  const { visibility: visibilityRaw } = props
  const { nodeMap, doCopy, doCut, doPaste, doDelete: doDeleteRaw, doRestore, doMove } = state
  const { category, scrollContainerRef, rootDirectoryId, backend } = state
